(function() {
	...
	var w = /:[0-9]+$/,
		y = function(a, b) {
			...
			switch (b) {
				...
				case 'port':
					a = String(
						Number(a.hostname ? a.port : t.location.port) ||
							('http' == c ? 80 : 'https' == c ? 443 : '')
					);
					break;
				case 'path':
	...
	function A() {
		for (var a = B, b = {}, c = 0; c < a.length; ++c) b[a[c]] = c;
		return b;
	}
	function C() {
				...
				if (a) {
					...
					if (e && '1' === e[1]) {
						...
						if (h) {
							b = {};
							var p = f ? f.split('*') : [];
							for (f = 0; f < p.length; f += 2) b[p[f]] = F(p[f + 1]);
							return b;
						}
					}
				}
	...
	function V(a, b, c) {
		b.href &&
			((a = S(a, b.href, void 0 === c ? !1 : c)), r.test(a) && (b.href = a));
	}
	function U(a, b) {
		if (b && b.action) {
	...
	l('google_tag_data.glBridge.decorate', function(a, b, c) {
		...
		a = N(a);
		if (b.tagName) {
			if ('a' == b.tagName.toLowerCase()) return V(a, b, c);
			if ('form' == b.tagName.toLowerCase()) return U(a, b);
		}
		if ('string' == typeof b) return S(a, b, c);
	});
			...
			a &&
				(c
					? ((c = ''),
					  b && f.test(b) && (c = ' id="' + b + '"'),
					  f.test(a) &&
							M.write('<script' + c + ' src="' + a + '">\x3c/script>'))
						...
						try {
							...
							if (1 > a.length) ge('xhr', 'ver', '0'), c();
								...
								if ('d' == b)
									...
									if (w)
										if (/^[a-z.]{1,6}$/.test(w)) {
											var ha = 'https://www.google.%/ads/ga-audiences'.replace(
												'%',
												w
											);
											wc(ha, d.google, ua);
	...
	Dc.prototype.S = function(a, b, c) {
		function d(c) {
			try {
				...
				('http:' == g.protocol || 'https:' == g.protocol) &&
					sd(a, g.hostname || '') &&
					g.href &&
			...
			if ('https:' != M.location.protocol && !Ba) {
				a: {
					...
					for (var c = 0; c < b.length && 100 > c; c++) {
						...
						if (
							d &&
							0 == d.indexOf('https://www.google-analytics.com/analytics')
						) {
							b = !0;
							break a;
						}
