(function() {
	...
	TagSettings = (function() {
		...
		d.prototype._get_page_url = function() {
			...
			else return this.context.win.location;
		};
		...
		d.prototype._set_partner_ids = function() {
			...
			for (f = g.length; e < f; e++) {
				c = g[e];
				if ((a = this.context.win[c + 'data_partner_id'])) b = b.concat(a);
				if ((a = this.context.win[c + 'data_partner_ids'])) b = a.concat(b);
			}
			...
			return this._set_options(
				...
				(function(a) {
					return function(b) {
						...
						for (f = g.length; e < f; e++) {
							c = g[e];
							if (a.context.win[c + b] != null) return a.context.win[c + b];
						}
					};
				})(this)
			);
		...
		d.prototype._set_options = function(a, b) {
			...
			for (c in a) {
				e = a[c];
				e = b(e);
				this._valid_property(e)
					? f.push(this._set_option(c, e))
					: f.push(void 0);
			}
			return f;
		};
		d.prototype._valid_property = function(a) {
			return (
				a != null && a !== 'undefined' && a.toString && a.toString().length > 0
			);
		};
		return d;
	})();
	...
	ParamBuilder = (function() {
		...
		d.prototype.add_default_parameters = function() {
			...
			a = { pid: c.partner_ids.toString(), url: c.page_url, fmt: a };
			e = [];
			for (b in a) {
				c = a[b];
				e.push(this.add(b, c));
		...
		d.prototype.add_encrypted_data_if_present = function() {
			...
			if (a != null) return this.add_all('ed', a);
		};
	...
	dom_ready = (function() {
		...
		return function(a) {
			...
			if (document.readyState === 'complete') return c();
	...
	PartnerTags = (function() {
		...
		d.fire_partners = function(a, b, c, e) {
			var f, g, h, i;
			if (!(a != null && a.length && a.length > 0)) return null;
			i = [];
			g = 0;
		...
		d.fire_partner_pixel = function(a, b, c, e) {
			...
			a = a.u;
			e.now().getTime();
			return b.write_pixel({ src: a, onload: function() {} });
		};
		d.fire_partner_script = function(a, b, c, e) {
			a.pid.toString();
			a = a.u;
			e.now().getTime();
			return b.write_script({ src: a, onload: function() {} });
		};
		return d;
	})();
	var FirstPartyIdMgr;
	FirstPartyIdMgr = (function() {
		function d(c, e) {
			...
			if (
				(this.li_fat_id = this._get_param_value_from_url(
					e.data.page_url_query,
					b
				))
			) {
				f = e.data.context_hostname.split('.');
				this._try_to_set_li_fat_cookie_on_highest_top_level_domains(f, []);
			} else this.li_fat_id = this.cookie_manager.get(a);
		}
		var a, b;
		a = b = 'li_fat_id';
		d.prototype._try_to_set_li_fat_cookie_on_highest_top_level_domains = function(
			c,
			e
		) {
			var f;
			if (c.length === 0) return this._set_li_fat_cookie();
			else if (e.length === 0) {
				e.unshift(c.pop());
		...
		d.prototype._get_param_value_from_url = function(c, e) {
			...
			for (j = g.length; 0 <= j ? i < j : i > j; f = 0 <= j ? ++i : --i) {
				...
				f = h[0];
				h = h[1];
				if (f === e) return decodeURIComponent(h);
			}
		};
		return d;
	})();
	var BaseLogger,
		...
		__extends = function(d, a) {
			function b() {
				this.constructor = d;
			}
			for (var c in a) if (__hasProp.call(a, c)) d[c] = a[c];
	...
	Tag = (function() {
		...
		d.prototype.to_html_string = function() {
			...
			c += '>';
			if (this.attributes.text != null) c += '' + this.attributes.text;
		...
		d.prototype.set_invisible = function() {
			var a;
			if ((a = this.attributes).style == null) a.style = {};
			return (this.attributes.style.display = 'none');
		};
		d.prototype.set_property = function(a, b, c) {
			return (!this._is_object(c)
				? this._set_single_property
				: this._set_multiple_properties)(a, b, c);
		};
	...
	HashUtil = (function() {
		...
		d.merge = function(a, b) {
			var c;
			for (c in a) if (a.hasOwnProperty(c) && b[c] == null) b[c] = a[c];
			return b;
		};
			...
			for (b in a) {
				...
				if (g != null)
					if (ArrayUtil.is_array(g)) {
						...
						c.push(e);
					} else c.push(this._to_param(b, g));
			}
	...
	CookieManager = (function() {
		...
		d.prototype.get = function(a) {
			...
			for (f = g.length; e < f; e++) {
				...
				b = c[0];
				c = c[1];
				if (b === a && c != null) return unescape(c);
			}
			return null;
		};
	...
	InsightTags = (function() {
		...
		d.init = function(a) {
			...
			b != null && b();
			if (this.already_called(a.win)) return 'already called';
		...
		d.prototype.get_pixli_url = function() {
			return (
				'' +
				this.tag_settings.data.protocol +
				'//' +
				(this.tag_settings.data.test_url || 'px.ads.linkedin.com') +
				'/collect/'
			);
		};
		...
		d.prototype.write_tags_to_dom = function() {
			...
			if (this.tag_settings.data.use_iframe != null)
				this.dom.write_iframe({ id: '_bizo_iframe', src: a });
			else {
				if (this.tag_settings.data.async_target != null)
					b = this.context.doc.getElementById(
						this.tag_settings.data.async_target
					);
				return this.dom.write_script({ src: a }, b);
			}
		};
		d.prototype.attach_global_functions_for_data_collector_callback = function(
			a
		) {
			if (a == null) a = this.context.win;
			return (a._bizo_fire_partners = (function(b) {
				return function(c) {
					try {
						return PartnerTags.fire_partners(
							c,
							b.dom,
							b.tag_settings,
							b.context.clock
		...
		d.prototype.init = function() {
			var a, b, c, e, f;
			f = this.exit_conditions = [
				['no partner id', this.tag_settings.data.partner_ids == null],
			];
			c = 0;
			for (e = f.length; c < e; c++) {
				...
				a = a[1];
				if (a === true) {
					ErrorLog.log(this.context, { error: b }, ConsoleLogger, LocalLogger);
					return b;
				}
			}
