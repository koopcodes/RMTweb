...
window.Typekit.config = {
	a: '3932814',
	c: ['.tk-aktiv-grotesk', '"aktiv-grotesk",sans-serif'],
	fi: [14032, 14033, 14034, 14035, 21510, 21514],
	fc: [
		{
			id: 14032,
			family: 'aktiv-grotesk',
			src:
				'https://use.typekit.net/af/affdf5/00000000000000003b9b0328/27/{format}{?primer,subset_id,fvd,v}',
			descriptors: {
				weight: '400',
				style: 'normal',
				primer:
					'7fa3915bdafdf03041871920a205bef951d72bf64dd4c4460fb992e3ecc3a862',
			},
		},
		{
			id: 14033,
			family: 'aktiv-grotesk',
			src:
				'https://use.typekit.net/af/90ce96/00000000000000003b9b0329/27/{format}{?primer,subset_id,fvd,v}',
			descriptors: {
				weight: '400',
				style: 'italic',
				primer:
		...
		},
		{
			...
			family: 'aktiv-grotesk',
			src:
				'https://use.typekit.net/af/b5ce2a/00000000000000003b9b032d/27/{format}{?primer,subset_id,fvd,v}',
			descriptors: {
				weight: '700',
				style: 'normal',
				primer:
		...
		},
		{
			...
			family: 'aktiv-grotesk',
			src:
				'https://use.typekit.net/af/3e6f7f/00000000000000003b9b032c/27/{format}{?primer,subset_id,fvd,v}',
			descriptors: {
				weight: '700',
				style: 'italic',
				primer:
		...
		},
		{
			...
			family: 'aktiv-grotesk',
			src:
				'https://use.typekit.net/af/97f831/000000000000000000017875/27/{format}{?primer,subset_id,fvd,v}',
			descriptors: {
				weight: '800',
				style: 'normal',
				primer:
		...
		},
		{
			...
			family: 'aktiv-grotesk',
			src:
				'https://use.typekit.net/af/f0f4a0/000000000000000000017877/27/{format}{?primer,subset_id,fvd,v}',
			descriptors: {
				weight: '900',
				style: 'normal',
				primer:
	...
	fn: ['aktiv-grotesk', ['i4', 'i7', 'n4', 'n7', 'n8', 'n9']],
	hn: 'use.typekit.net',
	ht: 'tk',
	js: '1.19.2',
	kt: 'ynk7ybf',
...
/*{"k":"1.19.2","auto_updating":true,"last_published":"2018-10-22 22:12:17 UTC"}*/
(function(window, document, undefined) {
	...
	function ra(a, b) {
		if ('i' === b) {
			...
			for (var e in c) {
				for (
					var f = [400, 300, 200, 100, 500, 600, 700, 800, 900], g = 400, k = 0;
					k < f.length;
					k++
				)
					if (((g = f[k]), c[e][g])) {
	...
	function za(a) {
		...
		d
			? l() - a.F >= a.I
				? ya() && Aa(a, b, c) && (!a.A || a.A.hasOwnProperty(a.b.b))
					? Ba(a, a.D)
					: Ba(a, a.H)
				: Ca(a)
			: Ba(a, a.D);
				...
				if (null === Q)
					if (window.FontFace) {
						var e = /Gecko.*Firefox\/(\d+)/.exec(window.navigator.userAgent),
							pa = /AppleWebKit\/([0-9]+)(?:\.([0-9]+))(?:\.([0-9]+))/.exec(
								window.navigator.userAgent
							);
						Q = e
							? 42 < parseInt(e[1], 10)
							: pa && /Apple/.exec(window.navigator.vendor)
							? 603 <= parseInt(pa[1], 10)
							: !0;
					} else Q = !1;
				Q
					? (c = new Da(
	...
	for (
		var U, m = window.Typekit.config || {}, Oa = [], V = m.fc, Qa = 0;
		Qa < V.length;
		Qa++
	)
		Oa.push(V[Qa].id);
	...
	function Ua(a, b) {
		...
		for (var e = 0; e < c.length; e++)
			if (
				c[e].family === a &&
				c[e].descriptors.weight === d.weight &&
				c[e].descriptors.style === d.style &&
				1 === c[e].descriptors.subset_id
			)
				...
				if (!window.XDomainRequest) {
					...
					a.onload = function() {
						for (var a = !1, b = 0; b < document.styleSheets.length; b++)
							if (
								null === document.styleSheets[b].href &&
								/ghostery-purple-box/.test(
									document.styleSheets[b].ownerNode.textContent
								)
							) {
								a = !0;
								break;
			...
			a.fontactive = function(a, b) {
				...
				a: {
					...
					for (var k = 0; k < V.length; k++)
						if (
							V[k].family === a &&
							V[k].descriptors.weight === g.weight &&
							V[k].descriptors.style === g.style
						) {
							g = V[k].id;
							break a;
						}
